...
if (!have_posts()) : 
	...
	$heading_alignment = $cmsmasters_option['magazilla' . '_heading_alignment'];

	echo '<div class="cmsmasters_search_zero' . ($heading_alignment =='center' ? ' tac' : ($heading_alignment =='right' ? ' tar fr' : '')) . '">' . 
		'<h4>' . esc_html__('Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'magazilla') . '</h4>' . 
		'<br />';
