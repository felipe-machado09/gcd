...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && magazilla_project_check_exc_cont()) ? true : false;
...
$project_thumb_size = (($cmsmasters_pj_layout_mode == 'masonry') ? 'cmsmasters-project-masonry-thumb' : 'cmsmasters-blog-masonry-thumb');
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	<div class="project_outer">
		<?php 
		echo '<div class="project_outer_image_wrap' . ($rollover ? ' with_rollover' : '') . '">';
		
			if ($title || $categories || $date) {
				echo '<div class="project_outer_image_wrap_meta entry-meta">';
					...
					echo '<div class="project_outer_image_wrap_meta_bottom">';

					$date ? magazilla_get_project_date() : "";

					$title ? magazilla_project_heading(get_the_ID(), 'h4', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';

					echo '</div>' . 
				'</div>';
			}
			
			magazilla_thumb_rollover(get_the_ID(), $project_thumb_size, false, false, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		
		echo '</div>';
		?>
		
		<?php 
		if ($excerpt || $likes || $comments) {
			echo '<div class="project_inner">';

				$excerpt ? magazilla_project_exc_cont() : '';
