...
if (
	$cmsmasters_option['magazilla' . '_profile_post_cat'] || 
	$cmsmasters_option['magazilla' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			magazilla_profile_title_nolink(get_the_ID(), 'h2', $cmsmasters_profile_subtitle, 'h5');
		echo '</header>';
	}
	...
	if ($profile_sidebar == 'true') {
		echo '<div class="profile_sidebar cmsmasters_column_sticky">';
		
			if ($profile_details == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					'<h4 class="profile_details_title"><span>' . esc_html($cmsmasters_profile_details_title) . '</span></h4>';
					
					magazilla_get_profile_likes('post');
					
					magazilla_get_profile_comments('post');
					
					magazilla_get_profile_features('details', $cmsmasters_profile_features, false, 'h4', true);
					
					magazilla_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			magazilla_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h4', true);
			
			magazilla_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h4', true);
			
			magazilla_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h4', true);
			
			
			magazilla_profile_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'magazilla'), 'h4');
		
			if ($cmsmasters_profile_sharing_box == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					'<h4 class="profile_details_title"><span>' . esc_html__('Like this profile?', 'magazilla') . '</span></h4>';
...
if ($cmsmasters_option['magazilla' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['magazilla' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['magazilla' . '_profile_post_nav_order_cat'] : false);
	
	magazilla_prev_next_posts($order_cat, 'pl-categs');
}
