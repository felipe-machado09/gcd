...
if (!function_exists('magazilla_system_fonts_list')) {
	function magazilla_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('magazilla_text_decoration_list')) {
	function magazilla_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'magazilla'), 
			'underline' => esc_html__('underline', 'magazilla'), 
			'overline' => esc_html__('overline', 'magazilla'), 
			'line-through' => esc_html__('line-through', 'magazilla'), 
		);
...
if (!function_exists('magazilla_custom_color_schemes_list')) {
	function magazilla_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'magazilla'), 
			'second' => esc_html__('Custom 2', 'magazilla'), 
			'third' => esc_html__('Custom 3', 'magazilla'),
			'fourth' => esc_html__('Bottom - Footer', 'magazilla')
		);
		
		
		return apply_filters('magazilla_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('magazilla_locate_template')) {
	function magazilla_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_magazilla_theme_style') ? get_option('cmsmasters_magazilla_theme_style') : ''));
...
if (!function_exists('magazilla_all_theme_styles')) {
	function magazilla_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('magazilla_all_color_schemes_list')) {
	function magazilla_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'magazilla'), 
			'header' => 		esc_html__('Header', 'magazilla'), 
			'navigation' => 	esc_html__('Navigation', 'magazilla'), 
			'header_top' => 	esc_html__('Header Top', 'magazilla'), 
			'footer' => 		esc_html__('Footer', 'magazilla') 
		);
...
if (CMSMASTERS_LEARNPRESS) {
	...
	function magazilla_change_template_for_learnpress() {
		return 'learnpress/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/templates';
	}
	
	add_filter('learn_press_template_path', 'magazilla_change_template_for_learnpress');
}
...
if (!function_exists('magazilla_load_theme_textdomain')) {
	function magazilla_load_theme_textdomain() {
		load_theme_textdomain('magazilla', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'magazilla_load_theme_textdomain')) {
	add_action('after_setup_theme', 'magazilla_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('magazilla_theme_activation')) {
	function magazilla_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'magazilla') {
			add_option('cmsmasters_active_theme', 'magazilla', '', 'yes');
...
add_action('after_switch_theme', 'magazilla_theme_activation');



// Framework Deactivation
if (!function_exists('magazilla_theme_deactivation')) {
	function magazilla_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'magazilla_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('magazilla_plugin_activation')) {
	function magazilla_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'magazilla_plugin_activation', 10, 2);


if (!function_exists('magazilla_plugin_activation_regenerate')) {
	function magazilla_plugin_activation_regenerate() {
...
add_action('init', 'magazilla_plugin_activation_regenerate');


function magazilla_run_reinit_import_options($post_id, $key, $value) {
...
add_action('import_post_meta', 'magazilla_run_reinit_import_options', 10, 3);