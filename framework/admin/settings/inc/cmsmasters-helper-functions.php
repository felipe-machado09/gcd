...
function magazilla_get_admin_page() {
	global $pagenow;
...
function magazilla_default_tab() {
	$current_page = magazilla_get_admin_page();
...
function magazilla_get_the_tab() {
	global $pagenow;
	
	$default_tab = magazilla_default_tab();
...
function magazilla_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'magazilla') . '</a>';
		}
